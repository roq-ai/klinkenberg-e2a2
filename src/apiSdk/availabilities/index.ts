...
export const getAvailabilities = async (query?: AvailabilityGetQueryInterface) => {
  const response = await axios.get(`/api/availabilities${query ? `?${queryString.stringify(query)}` : ''}`);
