...
function AppointmentListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AppointmentInterface[]>(
    ...
    () =>
      getAppointments({
        relations: ['user_appointment_student_idTouser', 'user_appointment_instructor_idTouser'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_appointment_student_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_appointment_instructor_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_appointment_student_idTouser?.id}`}>
                          {record.user_appointment_student_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_appointment_instructor_idTouser?.id}`}>
                          {record.user_appointment_instructor_idTouser?.email}
                        </Link>
                      </Td>
                    )}
